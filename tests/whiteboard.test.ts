...
describe('白板橡皮擦局部擦除', () => {
  it('擦到笔迹局部会保留对象并记录擦除点', () => {
    ...
    expect(result.items[0].erasePoints).toEqual([{ x: 0.3, y: 0.3, r: 0.02 }])
  })
  ...
  it('图片不会被橡皮擦误删,文字按内容范围可命中', () => {
    ...
    expect(eraseBoardItemsAt([image], { x: 0.2, y: 0.2 }).items.map((item) => item.id)).toEqual(['img'])
