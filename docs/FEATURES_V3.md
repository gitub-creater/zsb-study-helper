...
| 讲题语音 | 浏览器/系统原生 Web Speech API | 使用 `SpeechSynthesis`，不下载、内置或联网调用第三方声音包。 |
...
“AI 数字讲题”使用设备的 `SpeechSynthesis`：可以随时打开或关闭，支持朗读、暂停、继续、停止、重新播放、0.75/1/1.25/1.5 倍速、刷新声音和选择可用音色。默认优先普通话，朗读时当前句子会高亮；语音设置随本机状态保存，刷新和重新打开后仍在。设置页的“启用语音朗读”是总开关，AI 讲题页和每条安排任务还提供独立开关；关闭总开关会停止正在进行的语音朗读，但不会关闭任务单独选择的“短提示音”。
